...
Try {
	...
	[string]$appName = 'Public'
	[string]$appVersion = '10.5.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Install Tableau Public silently, do not trigger a restart, do not include a desktop shortcut, and remove previous versions (upgrade the version of Tableau Public).
		$exitCode = Execute-Process -Path "TableauPublicDesktop-64bit-10-5-0.exe" -Parameters "/quiet /norestart ACCEPTEULA=1 DESKTOPSHORTCUT=0 REMOVEINSTALLEDAPP=1" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-Process -Path "TableauPublicDesktop-64bit-10-5-0.exe" -Parameters "/uninstall /quiet /norestart" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
